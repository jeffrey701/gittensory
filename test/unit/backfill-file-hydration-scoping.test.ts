...
describe("GitHub PR file hydration scoping (#audit-rate-headroom)", () => {
  ...
  it("does not fetch PR files for a pull request missing from pull_requests", async () => {
    ...
    const result = await refreshPullRequestDetails(env, "JSONbored/loopover", 404);
  ...
  it("does not re-fetch files for a closed PR that already has complete stored telemetry, but does when forced", async () => {
    ...
    const unforced = await refreshPullRequestDetails(env, "JSONbored/loopover", 20);
    ...
    const forced = await refreshPullRequestDetails(env, "JSONbored/loopover", 20, { force: true });
  ...
  it("still fetches a closed PR without a complete sync state (no stored telemetry yet to rely on)", async () => {
    ...
    const result = await refreshPullRequestDetails(env, "JSONbored/loopover", 21);
  ...
  it("fetches PR files for a current open PR when no repo+PR+head snapshot exists", async () => {
    ...
    const result = await refreshPullRequestDetails(env, "JSONbored/loopover", 30);

    expect(result).toMatchObject({ status: "complete" });
    expect(urls.some((url) => url.includes("/pulls/30/files"))).toBe(true);
    expect(await listPullRequestFiles(env, "JSONbored/loopover", 30)).toEqual([expect.objectContaining({ path: "src/a.ts" })]);
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 30)).toMatchObject({ headSha: "head-1", status: "complete" });
  });
  ...
  it("reuses the repo+PR+head file snapshot without calling GitHub when the head is unchanged", async () => {
    ...
    const result = await refreshPullRequestDetails(env, "JSONbored/loopover", 31);

    expect(result).toMatchObject({ status: "complete", warnings: [] });
    expect(urls.some((url) => url.includes("/pulls/31/files"))).toBe(false);
    expect(await listPullRequestFiles(env, "JSONbored/loopover", 31)).toEqual([expect.objectContaining({ path: "src/cached.ts", changes: 3 })]);
  });
  ...
  it("fetches fresh files and does not reuse the previous snapshot when the head SHA changes", async () => {
    ...
    const result = await refreshPullRequestDetails(env, "JSONbored/loopover", 32);

    expect(result).toMatchObject({ status: "complete" });
    expect(urls.some((url) => url.includes("/pulls/32/files"))).toBe(true);
    expect(await listPullRequestFiles(env, "JSONbored/loopover", 32)).toEqual([expect.objectContaining({ path: "src/new.ts" })]);
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 32)).toMatchObject({ headSha: "head-2" });
  });
  ...
  it("does not stamp a new head-SHA file snapshot when the changed-head file fetch fails", async () => {
    ...
    const failed = await refreshPullRequestDetails(env, "JSONbored/loopover", 33);

    expect(failed).toMatchObject({ status: "partial", warnings: [expect.stringContaining("File sync failed for #33")] });
    expect(await listPullRequestFiles(env, "JSONbored/loopover", 33)).toEqual([expect.objectContaining({ path: "src/old.ts" })]);
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 33)).toMatchObject({
      headSha: "head-1",
      filesSyncedAt: "2026-05-20T00:00:00.000Z",
    });
    ...
    failFiles = false;
    urls.length = 0;
    const retried = await refreshPullRequestDetails(env, "JSONbored/loopover", 33);

    expect(retried).toMatchObject({ status: "complete", warnings: [] });
    expect(urls.some((url) => url.includes("/pulls/33/files"))).toBe(true);
    expect(await listPullRequestFiles(env, "JSONbored/loopover", 33)).toEqual([expect.objectContaining({ path: "src/new.ts" })]);
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 33)).toMatchObject({ headSha: "head-2" });
  });
  ...
  it("a failed file sync never regresses a marker a concurrent successful sync already advanced past it (gate review finding)", async () => {
    ...
    const failed = await refreshPullRequestDetails(env, "JSONbored/loopover", 34);
    ...
    // marker survives.
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 34)).toMatchObject({
      headSha: "head-3",
      filesSyncedAt: "2026-05-25T00:00:00.000Z",
    });
  });
  ...
  it("defers historical merged-PR file hydration when the REST budget is below the historical-backfill floor, while cheap metadata still syncs", async () => {
    ...
    const result = await backfillRepositorySegment(env, { repoFullName: "JSONbored/loopover", segment: "recent_merged_pull_requests", mode: "full" });
    ...
    expect(urls.some((url) => url.includes("/files"))).toBe(false);
    const stored = await listRecentMergedPullRequests(env, "JSONbored/loopover");
  ...
  it("still allows current open PR convergence within its small batch cap at the same reduced budget", async () => {
    ...
    const result = await backfillOpenPullRequestDetails(env, { repoFullName: "JSONbored/loopover", mode: "light", cursor: 0 });
  ...
  it("caps historical merged-PR file hydration per page even when the REST budget is healthy", async () => {
    ...
    await backfillRepositorySegment(env, { repoFullName: "JSONbored/loopover", segment: "recent_merged_pull_requests", mode: "light" });
  ...
  it("records bounded caller labels on the PR files fetch metric for backfill, historical, and live-review callers", async () => {
    ...
    await backfillOpenPullRequestDetails(env, { repoFullName: "JSONbored/loopover", mode: "light", cursor: 0 });
    await upsertPullRequestFromGitHub(env, "JSONbored/loopover", {
      number: 51,
    ...
    await refreshPullRequestDetails(env, "JSONbored/loopover", 51);
    await backfillRepositorySegment(env, { repoFullName: "JSONbored/loopover", segment: "recent_merged_pull_requests", mode: "light" });
  ...
  it("populates the head-SHA snapshot from the monolithic backfillRegisteredRepositories path so a later run reuses it (regression: the /run admin endpoint's PR-detail loop must WRITE the cache it reads)", async () => {
    ...
    await upsertRepositoryFromGitHub(env, { name: "loopover", full_name: "JSONbored/loopover", private: true, owner: { login: "JSONbored" } }, 456);
    let filesCallCount = 0;
    stubFetchTracking((url) => {
      if (url.endsWith("/repos/JSONbored/loopover")) return Response.json({ name: "loopover", full_name: "JSONbored/loopover", default_branch: "main", owner: { login: "JSONbored" } });
    ...
    await backfillRegisteredRepositories(env, { repoFullName: "JSONbored/loopover", limits: { issues: 10, pullRequests: 10, recentMergedPullRequests: 10, pullRequestDetails: 10 } });
    expect(filesCallCount).toBe(1);
    expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 60)).toMatchObject({ headSha: "head-60", status: "complete" });

    // Second run, same head: the cache this path itself just wrote must be reused, not re-fetched.
    await backfillRegisteredRepositories(env, { repoFullName: "JSONbored/loopover", force: true, limits: { issues: 10, pullRequests: 10, recentMergedPullRequests: 10, pullRequestDetails: 10 } });
    expect(filesCallCount).toBe(1);
  });
  ...
  describe("upsertPullRequestDetailSyncState partial-update contract", () => {
    it("preserves an existing headSha (and other fields) when a later upsert omits them, as every 'running' pre-fetch stamp does", async () => {
      ...
      await upsertPullRequestDetailSyncState(env, { repoFullName: "JSONbored/loopover", pullNumber: 90, status: "running" });

      expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 90)).toMatchObject({
        status: "running",
        headSha: "sha-a",
        filesSyncedAt: "2026-05-20T00:00:00.000Z",
        errorSummary: "prior warning",
    ...
    it("clears headSha when a caller explicitly passes null, distinguishing 'omitted' from 'cleared'", async () => {
      const env = createTestEnv();
      await upsertPullRequestDetailSyncState(env, { repoFullName: "JSONbored/loopover", pullNumber: 91, status: "complete", headSha: "sha-b" });

      await upsertPullRequestDetailSyncState(env, { repoFullName: "JSONbored/loopover", pullNumber: 91, status: "complete", headSha: null });

      expect(await getPullRequestDetailSyncState(env, "JSONbored/loopover", 91)).toMatchObject({ headSha: null });
    });
  });
  ...
  describe("listPullRequestFiles ordering (#linked-issue-satisfaction-cache-fingerprint-stability)", () => {
    it("REGRESSION: returns files in a stable path order regardless of write order, so a downstream content fingerprint never flips for an unchanged PR", async () => {
      ...
      const first = await listPullRequestFiles(env, "JSONbored/loopover", 200);
      const second = await listPullRequestFiles(env, "JSONbored/loopover", 200);
